...
## 11. Cadeia de hotfixes de infraestrutura e isolamento DEV × produção (2026-07-28) — ponteiro

Após a amostragem da seção 10, uma cadeia separada de hotfixes de infraestrutura (harness CRLF do teste do controle DEV, instrução preventiva global do retry do resumo, isolamento do endpoint de pagamento `criarPreferencia` entre `revalidapro-dev` e `revalidapro-f812e`) foi aplicada e auditada. Narrativa completa, baseline Git exato e sequência de retomada em `CHECKPOINT_SUPERAPOSTAS_2026_2_ISOLAMENTO_DEV_2026_07_28.md` (continua de onde `CHECKPOINT_SUPERAPOSTAS_2026_2_GOVERNANCA_RESUMOS_NUMERICOS_2026_07_28.md`, seções 10–13, termina). R067/R092/R077 permanecem no mesmo estado desta seção 10 — nenhum deles foi tocado pela cadeia de infraestrutura.

## 12. Piloto Controlado DEV — hotfix implementado, geração real ainda não autorizada (2026-07-29)

A auditoria de governança numérica referenciada na seção 10 (achado sobre `validarResumoSA`) **foi concluída** — decisão arquitetural registrada em `CHECKPOINT_SUPERAPOSTAS_2026_2_GOVERNANCA_RESUMOS_NUMERICOS_2026_07_28.md` seção 10 (manter zero-tolerância a números sem grounding; Estratégia B explicitamente adiada). A razão operacional que motivou a pausa da amostragem (seção 10 acima) portanto não bloqueia mais R092/R077 do ponto de vista de governança numérica.

- **R092** (Aleitamento materno: fissura/ingurgitamento) — classificado `LIBERÁVEL QUALITATIVO`/`DISPENSAVEL`/risco `BAIXO` na matriz de grounding; **liberado para piloto controlado** (menor risco numérico entre os dois recortes pendentes); **ainda não foi gerado**.
- **R077** (Quedas em idosos) — mesma classificação (`LIBERÁVEL QUALITATIVO`/`DISPENSAVEL`/risco `BAIXO`); **liberável, mas não escolhido** para o piloto (risco numérico marginalmente maior — tentação de citar escalas/escores de risco de queda).
- **R067** (Parassonias, `SA_2026_2_Q28`) — questão aprovada e salva; **não deve ser recriada**. Resumo continua rejeitado/pendente, fora do escopo desta missão.
