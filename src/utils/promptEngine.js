...
- EVITE termos absolutos que soam como citação de diretriz sem fonte real: "patognomônico", "padrão-ouro"/"padrão ouro", percentuais específicos ("reduz mortalidade em 30%"), "desde [ano]". O mesmo vale, mesmo fora de citação nomeada, para os termos categóricos "sempre", "nunca", "obrigatório"/"obrigatória"/"obrigatórios"/"obrigatórias" e "em todos os casos"/"em todos os pacientes" — sem diretriz controlada injetada, nenhum desses termos pode ser usado em raciocínio, tto, dicaMestre ou nas notas das alternativas.
...
PRECEDÊNCIA OBRIGATÓRIA — SEM EXCEÇÃO: quando NÃO houver bloco "DIRETRIZ CONTROLADA" injetado nesta geração, SA-3 e esta SA-4 se sobrepõem à REGRA 2 (inclusive à instrução de nomear a fonte e informar dose/periodicidade na nota da alternativa correta) e à seção DIRETRIZES ATUALIZADAS/Cadernos de Atenção Primária — mesmo que essas duas pareçam pedir o contrário: o campo "tto" continua proibido de conter posologia numérica, e "ano_diretriz"/"fonte_diretriz" continuam null/"".
...
═══ REGRA SA-6 — NÃO INTERVENÇÃO LEGÍTIMA — EXCLUSIVA SUPER APOSTAS 2026.2 ═══
Quando clinicamente indicado e sustentado pela conduta/diretriz aplicável ao tema, a alternativa correta PODE ser uma decisão de não intervir, em vez de sempre uma conduta ativa. Isso inclui, quando pertinente ao tema sorteado: observação clínica; orientação/tranquilização sem intervenção adicional; seguimento/acompanhamento programado; conduta expectante; desprescrição de medicação inadequada ou desnecessária; suspensão de uma intervenção em curso que não é mais necessária; decisão fundamentada de não tratar ou não solicitar determinado exame.
Isso é uma POSSIBILIDADE, não uma obrigação: use somente quando o tema sorteado realmente comportar esse desfecho. NÃO force uma resposta de não intervenção em temas de urgência/emergência, nem distorça o quadro clínico só para caber essa resposta.
PROIBIDO: não intervenção diante de sinais de alarme, instabilidade, risco iminente ou achados que exigiriam ação segundo a conduta/diretriz vigente — não intervir só é resposta correta quando é, de fato, a conduta mais segura e adequada para aquele caso.
PROIBIDO: contrariar diretriz vigente, protocolo de urgência ou segurança do paciente para encaixar artificialmente uma resposta de não intervenção.
A alternativa de não intervenção segue TODAS as regras da SA-1 (anti-pistas): não pode ser identificável pela forma — não pode ser a mais curta, a mais vaga ou a única sem justificativa robusta. A nota que a acompanha precisa ser tão fundamentada quanto a de qualquer conduta ativa.

═══ REGRA SA-7 — CENÁRIOS AMPLIADOS: VIGILÂNCIA, TERRITÓRIO E REDE — EXCLUSIVA SUPER APOSTAS 2026.2 ═══
Além dos cenários já previstos (UBS/ESF, UPA/PS, enfermaria, pré-natal/GO), quando pertinente ao tema, o caso pode se passar em contextos de: comunidade e território adstrito; vigilância epidemiológica; notificação compulsória; investigação de surto; saúde do trabalhador; articulação intersetorial; referência e contrarreferência entre pontos de atenção; organização da rede de atenção; limitação real de recursos em unidade de menor complexidade, exigindo decisão sobre transferência.
Esses cenários são OPCIONAIS — use apenas quando o tema realmente comportar esse contexto (ex.: doença de notificação compulsória, saúde do trabalhador, medicina de família e comunidade, organização de rede). NÃO force nenhum deles em temas sem relação natural com esse contexto (ex.: não insira vigilância epidemiológica numa questão de arritmia cardíaca só para "cobrir" o critério).
PROIBIDO: cenário decorativo — o contexto de território/vigilância/rede só pode aparecer se influenciar de fato a decisão pedida na questão. Se removê-lo não muda a resposta correta, ele não deveria estar lá.
Esses cenários são ADITIVOS: não substituem UBS/ESF, UPA/PS, enfermaria e pré-natal/GO, que continuam sendo as opções padrão sempre que forem as mais adequadas ao tema.
Não use esses cenários para inflar artificialmente o enunciado — os limites de extensão desta seção (ver LIMITES DE TAMANHO OBRIGATÓRIOS) continuam valendo integralmente.`;
